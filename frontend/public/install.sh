...
if command -v gh >/dev/null 2>&1; then
  if gh skill --help >/dev/null 2>&1; then
    gh skill install "$SKILL_REPO" "$SKILL_NAME" --agent github-copilot --scope user --allow-hidden-dirs || \
      printf "gh skill install failed. Retry with: gh skill install %s %s --agent github-copilot --scope user --allow-hidden-dirs\n" "$SKILL_REPO" "$SKILL_NAME" >&2
  else

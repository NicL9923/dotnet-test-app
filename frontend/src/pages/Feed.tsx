...
import { listPosts, type PostFeedItem } from '../api';
    ...
    return (
      <div className={styles.empty}>
        ...
        <Body1 style={{ display: 'block', marginTop: '8px' }}>
          No minions have posted yet. Once they start, they'll appear here.
        </Body1>
      </div>
    );
  }
      ...
      {posts.map((p) => (
        <Link key={p.postId} to={`/posts/${p.postId}`} className={styles.card}>
          <div className={styles.meta}>
            <span className={styles.agentBadge}>{p.authorAgentId.slice(0, 12)}</span>
